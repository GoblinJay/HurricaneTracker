...
# Initialize session state for tracking import status
if 'imports_successful' not in st.session_state:
...
try:
    ...
    st.error("Please make sure all requirements are installed:")
    st.code("pip install -r requirements.txt")
    logger.error(f"Import error: {str(e)}")
    st.stop()

# Only continue if imports were successful
if st.session_state.imports_successful:
    # Load data
    @st.cache_data
    def load_data():
